...
$(SWAGGER): $(TOOLS_DIR)/go.mod ## Build swagger tool
	cd $(TOOLS_DIR); GOTOOLCHAIN=auto go build -trimpath -tags=tools -o $(TOOLS_BIN_DIR)/swagger github.com/go-swagger/go-swagger/cmd/swagger

$(GOCOVMERGE): $(TOOLS_DIR)/go.mod ## Build gocovmerge tool
	cd $(TOOLS_DIR); GOTOOLCHAIN=auto go build -trimpath -tags=tools -o $(TOOLS_BIN_DIR)/gocovmerge github.com/wadey/gocovmerge
